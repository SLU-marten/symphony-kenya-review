...
```bash
# Preprocessing (regenerates public/data/layers.json + maps/)
pip install -r scripts/requirements.txt           # one-time
python scripts/preprocess.py                      # full run: JSON + 20 PNGs at native 2541x1447
python scripts/preprocess.py --json-only          # only rebuild layers.json (also recomputes bounds)
python scripts/preprocess.py --maps-only          # skip JSON
python scripts/preprocess.py --theme ecosystem --band 1   # single band
python scripts/preprocess.py --low-res            # 2x downsampled (~3 MB total)
...
metadata_*.csv        →  parse_metadata           →   layers.json           ─┤  copies public/
(reference TIFF       →  compute_bounds_latlon    →   layers.json.bounds   ─┘  → gh-pages branch
  bounds)                  via transform_bounds
```

The source TIFFs and CSV live **outside this repo** at `../Layers_260427_250m/` (sibling folder, not in git, ~560 MB). On a fresh clone the contents of `public/data/maps/` don't exist — gitignored on `main`. Vite copies them into `dist/` during build, so the `gh-pages` branch always has them. To work locally you must first run `python scripts/preprocess.py`.

`layers.json` is now wrapped: `{ bounds: [[south, west], [north, east]], layers: [...] }`. `bounds` is in EPSG:4326 (lat/lon) — the format Leaflet's `imageOverlay` expects.
...
`render_band()` masks `arr <= 0.0` to alpha=0 so the tile basemap shows through. In the source TIFFs, `0` is a real value (no presence / no pressure), not nodata. A transparent pixel in the displayed PNG ≠ no data in the source.
